...
void autonomous() {
    if (selected_auton == 1) {
		...
		pros::delay(1000);
		chassis.moveToPoint(-23.031, 23.146, 4000, {.maxSpeed=30}, false); // 2nd point
		pros::delay(3000); // delay to intake the blocks
		chassis.turnToHeading(136.1, 3000); // turn to face 3rd point
		pros::delay(2000);
		chassis.moveToPoint(-13.984, 13.745, 3000, {.maxSpeed = 30}); // 3rd point
		pros::delay(2000);
		...
		chassis.moveToPoint(-3.257, 43.548, 4000, {.maxSpeed = 30}, false); // move to 6th point
		pros::delay(2000);
